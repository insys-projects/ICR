// Icr0096.cpp : Defines the initialization routines for the DLL.
//
...
ICR_CfgAdc g_AdcCfg = { ADC_CFG_TAG, 14, 0, 0, 12, 1, 1000, 1800000000, 500};

ICR_CfgAdm g_AdmCfg = { ADM_CFG_TAG, 168, 0, 2, 5, 0, 500, 1, 0x49, 10000000, 1400000000,
						1, 1, 0, 0x4C,
...
BEGIN_MESSAGE_MAP(CIcr0096App, CWinApp)
END_MESSAGE_MAP()


// CIcr0096App construction

CIcr0096App::CIcr0096App()
{
...
// The one and only CIcr0096App object

CIcr0096App theApp;


// CIcr0096App initialization

BOOL CIcr0096App::InitInstance()
{
...
SUBMOD_API void __stdcall SUBMOD_GetInfo(int* pNumDev, PSUBMOD_INFO pDevInfo)
{
	...
	{
	case 0:
		lstrcpy(pDevInfo->sName, _T("FM212x1G"));
		pDevInfo->Type = FM212x1G;
		break;
	default:
		*pNumDev = -1;
		return;
		...
		case ADM_CFG_TAG:
			{
				...
				g_AdmCfg.bTempType	 = pAdmCfg->bTempType;	
				g_AdmCfg.bTempAdr	 = pAdmCfg->bTempAdr;	

				{
					...
					for( iiP=0; iiP<2; iiP++ )
					{
						g_AdmCfg.awRangeDeviation[iiP][iiAdc][iiRange] = pAdmCfg->awRangeDeviation[iiP][iiAdc][iiRange]; 
						g_AdmCfg.awBiasDeviation[iiP][iiAdc][iiRange]  = pAdmCfg->awBiasDeviation[iiP][iiAdc][iiRange];
					}
				}
...
SUBMOD_API int __stdcall SUBMOD_GetProperty(PSUBMOD_INFO pDeviceInfo)
{
	...
	pAdmCfg->bTempType	 = g_AdmCfg.bTempType;	
	pAdmCfg->bTempAdr	 = g_AdmCfg.bTempAdr;	

	{
		...
		for( iiP=0; iiP<2; iiP++ )
		{
			pAdmCfg->awRangeDeviation[iiP][iiAdc][iiRange] = g_AdmCfg.awRangeDeviation[iiP][iiAdc][iiRange];
			pAdmCfg->awBiasDeviation[iiP][iiAdc][iiRange]  = g_AdmCfg.awBiasDeviation[iiP][iiAdc][iiRange];
		}
	}
...
//	int curNum = pDeviceInfo->Number;
	...
	dlg.m_TempType   = g_AdmCfg.bTempType;	
	dlg.m_TempAdr   = g_AdmCfg.bTempAdr;

	{
		...
		for( iiP=0; iiP<2; iiP++ )
		{
			dlg.m_awRangeDeviation[iiP][iiAdc][iiRange] = g_AdmCfg.awRangeDeviation[iiP][iiAdc][iiRange];
			dlg.m_awBiasDeviation[iiP][iiAdc][iiRange]  = g_AdmCfg.awBiasDeviation[iiP][iiAdc][iiRange];
		}
	}
	...
	if (nResponse == IDOK)
	{
		...
		if( 
			...
			g_AdmCfg.bTempType	 != dlg.m_TempType	||
			g_AdmCfg.bTempAdr	 != dlg.m_TempAdr
		)
			nResponse |= 0x100;
...
//		g_AdcCfg.Number = ;
...

		{
			...
			for( iiP=0; iiP<2; iiP++ )
			{
				g_AdmCfg.awRangeDeviation[iiP][iiAdc][iiRange] = dlg.m_awRangeDeviation[iiP][iiAdc][iiRange];
				g_AdmCfg.awBiasDeviation[iiP][iiAdc][iiRange]  = dlg.m_awBiasDeviation[iiP][iiAdc][iiRange];
			}
		}
	}
