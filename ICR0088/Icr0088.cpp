// Icr0088.cpp : Defines the initialization routines for the DLL.
//
...
ICR_CfgAdm m_AdmCfg = { ADM_CFG_TAG, 2, 0, 0};
...
BEGIN_MESSAGE_MAP(CIcr0088App, CWinApp)
END_MESSAGE_MAP()


// CIcr0088App construction

CIcr0088App::CIcr0088App()
{
...
// The one and only CIcr0088App object

CIcr0088App theApp;


// CIcr0088App initialization

BOOL CIcr0088App::InitInstance()
{
...
SUBMOD_API void __stdcall SUBMOD_GetInfo(int* pNumDev, PSUBMOD_INFO pDevInfo)
{
	...
	{
	case 0:
		lstrcpy(pDevInfo->Name, _T("ADMDIO32"));
		pDevInfo->Type = ADMDIO32;
		break;
	default:
		*pNumDev = -1;
		return;
		...
		case ADM_CFG_TAG:
			{
				...
				m_AdmCfg.bBufType = pAdmCfg->bBufType;
...
SUBMOD_API int __stdcall SUBMOD_GetProperty(PSUBMOD_INFO pDeviceInfo)
{
	...
	pAdmCfg->bBufType = m_AdmCfg.bBufType;
...
//	int curNum = pDeviceInfo->Number;
	...
	dlg.m_BufType = m_AdmCfg.bBufType;
	...
	if (nResponse == IDOK)
	{
		...
		//  dismissed with OK
		m_AdmCfg.bBufType = dlg.m_BufType;
	}
