...
class CIcrAdp201cP5Dlg : public CDialog
{
	DECLARE_DYNAMIC(CIcrAdp201cP5Dlg)

public:
	CIcrAdp201cP5Dlg(CWnd* pParent = NULL);   // standard constructor
	virtual ~CIcrAdp201cP5Dlg();
...
protected:
	...
	void SetMaskOnCheckBoxesJ(UINT mask, int idx);
	void SetMaskOnCheckBoxesA(UINT mask, int idx);

	void GetMaskOnCheckBoxes(UINT& mask, int idx);
	void GetMaskOnCheckBoxesJ(UINT& mask, int idx);
