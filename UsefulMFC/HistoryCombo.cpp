...
static void GetInifileString(LPCTSTR FileName, LPCTSTR SectionName, LPCTSTR ParamName, LPSTR defValue, LPSTR strValue, int strSize)
{
	GetPrivateProfileString(SectionName, ParamName, defValue, strValue, strSize, FileName);
	TCHAR* pChar = _tcschr(strValue, ';');
	if(pChar) *pChar = 0; // удалить комментарий из строки
	int str_size = lstrlen(strValue);
	...
	//pChar = _tcschr(strValue, ' ');
	//if(pChar) *pChar = 0; // удалить пробелы из строки
	//pChar = strchr(strValue, '\t');
	//if(pChar) *pChar = 0; // удалить табуляции из строки
}

// Записать параметр типа строка в ini-файл:
//    FileName - имя ini-файла (с путем, если нужно)
//    SectionName - название секции
//    ParamName - название параметра (ключа)
//    value - значение параметра
//    Comment - комментарий (если NULL, то сохраняется прежний комментарий, если он был)
void WriteInifileParam(LPCTSTR FileName, LPCTSTR SectionName, LPCTSTR ParamName, LPCTSTR value, LPCTSTR Comment)
{
	TCHAR Buffer[255];
	TCHAR BufComment[128] = "";
	if(Comment)
		//lstrcpy(BufComment, Comment); // если задан комментарий, то просто копируем его
		sprintf_s(BufComment, "\t;%s", Comment); // если комментарий задан
	else
	{  // ищем параметр в файле и комментарий к нему
		GetPrivateProfileString(SectionName, ParamName, "NONAND", Buffer, sizeof(Buffer), FileName);
		if(lstrcmp(Buffer, "NONAND"))
		{ // параметр есть в файле
			TCHAR* pChar = _tcschr(Buffer, ';');
			if(pChar)
			{ // комментарий есть в строке
				//lstrcpy(BufComment, pChar+sizeof(TCHAR));
				sprintf_s(BufComment, "\t;%s", pChar+sizeof(TCHAR));
			}
		}
	}
	sprintf_s(Buffer, "%s%s", value, BufComment); // между значением параметра и комментарием вставлен символ табуляции
...
CString CHistoryCombo::LoadHistory(LPCTSTR lpszFileName, LPCTSTR lpszSection, LPCTSTR lpszKeyPrefix, 
...
//  GetCurrentDirectory(sizeof(m_IniFilePath)/sizeof(TCHAR), m_IniFilePath);
//  _tcscat(m_IniFilePath, "\\");
//  _tcscat(m_IniFilePath, lpszFileName);
  _tcscpy_s(m_IniFilePath, lpszFileName);
...
//    sText = pApp->GetProfileString(m_sSection, sKey);
	//GetPrivateProfileString(m_sSection, sKey, "", m_Buffer, sizeof(m_Buffer), m_IniFilePath);
	GetInifileString(m_IniFilePath, m_sSection, sKey, "", m_Buffer, sizeof(m_Buffer));
	sText = m_Buffer;
    if (!sText.IsEmpty())
...
//    pApp->WriteProfileString(m_sSection, sKey, sText);
	LPTSTR msg_buffer = sText.GetBuffer(120);
	//WritePrivateProfileString(m_sSection, sKey, msg_buffer, m_IniFilePath);
	WriteInifileParam(m_IniFilePath, m_sSection, sKey, msg_buffer, NULL);
  }
...
//    CString sText = pApp->GetProfileString(m_sSection, sKey);
	//GetPrivateProfileString(m_sSection, sKey, "", m_Buffer, sizeof(m_Buffer), m_IniFilePath);
	GetInifileString(m_IniFilePath, m_sSection, sKey, "", m_Buffer, sizeof(m_Buffer));
    CString sText = m_Buffer;
    if (sText.IsEmpty())
      break;
//    pApp->WriteProfileString(m_sSection, sKey, NULL); // remove entry
	//WritePrivateProfileString(m_sSection, sKey, NULL, m_IniFilePath);
	WriteInifileParam(m_IniFilePath, m_sSection, sKey, NULL, NULL);
  }
  if (m_bSaveRestoreLastCurrent)
  {
    CString sText;
