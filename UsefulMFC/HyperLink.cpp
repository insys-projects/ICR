...
//
// Thanks to Pеl K. Tшnder for auto-size and window caption changes.
//
...
/*
static GetMsg(DWORD dwMessageId, CString& string) 
{
	LPVOID lpMsgBuf;
...
void CHyperLink::SetAltToolTips(BOOL bAltToolTips)
{
...
void CHyperLink::CreateToolTipText(LPCTSTR url)
{
	...
	if (m_bAltToolTips)
	{
		...
		{
//			GetMsg(MSG_GO_TO_SITE, m_strToolTip);
			m_strToolTip = "Vizit to our site ";
			if(GetRegistryLocalParams())
				m_strToolTip = "Посетите наш сайт ";
			url += 7;
		}
		else if ( _tcsicmp(_T("mailto:"), strURL.Left(7)) == 0 )
		{
			m_strToolTip = "Your remarks, suggestions end impressions send to e-mail ";
			if(GetRegistryLocalParams())
				m_strToolTip = "Присылайте свои замечания, предложения и впечатления по адресу ";
//			GetMsg(MSG_E_MAIL, m_strToolTip);
			url += 7;
		}
	}
